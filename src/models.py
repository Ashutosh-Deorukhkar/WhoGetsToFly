...
class AirlineType:
    a: np.ndarray   # a_i(s) for each slot index s=0..m-1
    beta: float     # beta_i (complementarity strength)
    k: int          # k_i (capacity)
    lam: float      # lambda_i (overload penalty)


def generate_slots(start_hour: int = 8, end_hour: int = 18) -> np.ndarray:
    """Returns slot indices. Example: 8..17 for 8am-6pm (10 slots)."""
    return np.arange(start_hour, end_hour, dtype=int)
...
def sample_airline_types(
    ...
    rng: np.random.Generator,
    base_scale: float = 100.0,
    eta_range: tuple = (0.1, 0.5),
) -> list:
    """
    Sample private types for n_airlines.

    Each airline i gets:
      a_i(s) ~ TruncatedNormal(0.6 * base_scale, 0.25 * base_scale)
      beta_i  = rho_i * mean(a_i),  rho_i ~ Uniform(0.0, 1.0)
      k_i     drawn from {1,2,3,4} by size class
      lam_i   = eta_i * mean(a_i),  eta_i ~ Uniform(eta_range)
    """
    m = len(slots)
    types = []
    ...
    for _ in range(n_airlines):
        ...
        types.append(AirlineType(a=a, beta=beta, k=k, lam=lam))
